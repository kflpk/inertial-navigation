...
# Global variables for storing the sensor data
acc_x_data = []
acc_y_data = []
acc_z_data = []

# Create a lock to ensure thread-safe access to the data
data_lock = threading.Lock()

# Function to update the plot
def update_plot():
    with data_lock:
        plt.plot(acc_x_data, label='Acc X')
        plt.plot(acc_y_data, label='Acc Y')
        plt.plot(acc_z_data, label='Acc Z')
        plt.legend()
        plt.draw()
        plt.pause(0.001)
